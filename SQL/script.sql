...
create trigger check_out_validation
  before DELETE
  on CART
  for each row
  BEGIN
...
create trigger credit_number_verification_insertion
  before INSERT
  on CREDIT_CARD
  for each row
  BEGIN
...
create trigger credit_number_verification_update
  before INSERT
  on CREDIT_CARD
  for each row
  BEGIN
...
create function get_orders_quantity(book_id int)
  returns int
  BEGIN
    declare sum int;
...
create procedure purchase(IN user_id int)
  BEGIN
...
create procedure remove_from_cart(IN user_id int, IN book_id int)
  BEGIN
    DECLARE rollback bool default 0;
    declare continue handler for SQLEXCEPTION set rollback = 1;
    start transaction;
...
create procedure sell(IN book_id int, IN user_id int, IN quantity int)
  BEGIN
    DECLARE rollback bool default 0;
    declare continue handler for SQLEXCEPTION set rollback = 1;
    start transaction;
    update BOOK set BOOK.quantity = BOOK.quantity - quantity where BOOK.book_id = book_id;
    insert into SALES values (book_id, user_id, current_date, quantity);
    if rollback
    then
      ROLLBACK;
    else
